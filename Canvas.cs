        ...
        public void Render(Scene scene)
        {
            ...
            for (int f = 0; f < scene.Figures.Count; f++)
            {
                if (scene.Figures[f].Pts.Count > 1)
                {
                    ...
                    Pen colorlinea = new Pen(Color.FromArgb(50,50,50));
                    g.DrawPolygon(colorlinea, scene.Figures[f].Pts.ToArray());
                    ...
                    g.FillEllipse(Brushes.Red, scene.Figures[f].Centroid.X-3, scene.Figures[f].Centroid.Y-3, 6, 6);//*/
                }
            }
            pct.Invalidate();
        }
